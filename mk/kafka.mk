...
kafka-topics-list:  ## List the kafka topics from the kafka container
	...
	$(DOCKER_COMPOSE) -f "$(DOCKER_COMPOSE_FILE)" exec -it kafka-broker \
...
kafka-topics-create:  ## Create the kafka topics in KAFKA_TOPICS
	...
	for topic in $(KAFKA_TOPICS); do \
	    $(DOCKER_COMPOSE) -f "$(DOCKER_COMPOSE_FILE)" exec -it kafka-broker \
	    /opt/kafka/bin/kafka-topics.sh --create --topic $$topic --bootstrap-server "$(KAFKA_BOOTSTRAP_SERVERS)"; \
	done
...
kafka-topics-describe:  ## Execute kafka-topics.sh for KAFKA_TOPICS
	for topic in $(KAFKA_TOPICS); do \
	    $(DOCKER_COMPOSE) -f "$(DOCKER_COMPOSE_FILE)" exec -it kafka-broker \
	      /opt/kafka/bin/kafka-topics.sh --describe --topic $$topic --bootstrap-server "$(KAFKA_BOOTSTRAP_SERVERS)"; \
	done
