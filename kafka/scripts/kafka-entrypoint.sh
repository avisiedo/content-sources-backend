...
function create_topics {
    ...
    do
        while ! "${KAFKA_HOME}/bin/kafka-topics.sh" --topic "${item}" --bootstrap-server "${KAFKA_BOOTSTRAP_SERVERS}" --describe; do
            "${KAFKA_HOME}/bin/kafka-topics.sh" --create --bootstrap-server "${KAFKA_BOOTSTRAP_SERVERS}" --topic "${item}"
            sleep 1
        done
    done
    ...
    for item in ${KAFKA_TOPICS}
    do
        "${KAFKA_HOME}/bin/kafka-topics.sh" --topic "${item}" --bootstrap-server "${KAFKA_BOOTSTRAP_SERVERS}" --describe
    done
